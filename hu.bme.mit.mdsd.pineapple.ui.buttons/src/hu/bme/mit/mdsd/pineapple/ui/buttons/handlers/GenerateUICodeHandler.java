...
import hu.bme.mit.mdsd.pineapple.dataflownet.ui.generator.UIGenerator;
...
public class GenerateUICodeHandler extends AbstractDataflowNetHandler {
    ...
    protected void handleFile(ExecutionEvent event, IFile file) {
        ...
        final UIGenerator generator = new UIGenerator();
        ...
        if (isSuccessfullyValidatedModelWithIncQuery && isValidDefault) {
            ...
            Job job = new Job("Generating Java UI code for model file " + file.getName() + "...") {
                @Override
                protected IStatus run(IProgressMonitor monitor) {
                    try {
                        generator.generate(resource);
                    } catch (Exception e) {
                        System.err.println("Error occured while generating Java code. Error message: " + e.getMessage());
                    }
            ...
            String message = "The Java UI code generation cannot be started while the model is not valid.";
